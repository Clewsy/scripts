	...
	if [ -s /sys/devices/virtual/dmi/id/chassis_type ]; then
		...
		if [ -n "${CHASSIS_TYPE}" ]; then
				...
				if [ -n "${CHASSIS_VENDOR}" ]; then
					echo -e "${COL}${BOLD}├─Chassis:${RESET} ${CHASSIS_TYPE}, version ${CHASSIS_VERSION} (${CHASSIS_VENDOR})"
				else
					echo -e "${COL}${BOLD}├─Chassis:${RESET} ${CHASSIS_TYPE}, version ${CHASSIS_VERSION}"
				fi
			elif [ -n "${CHASSIS_VENDOR}" ]; then
				echo -e "${COL}${BOLD}├─Chassis:${RESET} ${CHASSIS_TYPE} (${CHASSIS_VENDOR})"
			fi
		else
			echo -e "${COL}${BOLD}├─Chassis:${RESET} ${CHASSIS_TYPE}"
		fi
	else
		...
		if [ -n "${BOARD_NAME}" ]; then
			if [ -n "${BOARD_VERSION}" ]; then
				if [ -n "${BOARD_VENDOR}" ]; then
					echo -e "${COL}${BOLD}├─Motherboard:${RESET} ${BOARD_NAME}, version ${BOARD_VERSION} (${BOARD_VENDOR})"
				else
					echo -e "${COL}${BOLD}├─Motherboard:${RESET} ${BOARD_NAME}, version ${BOARD_VERSION}"
				fi
			elif [ -n "${BOARD_VENDOR}" ]; then
				echo -e "${COL}${BOLD}├─Motherboard:${RESET} ${BOARD_NAME} (${BOARD_VENDOR})"
			else
				echo -e "${COL}${BOLD}├─Motherboard:${RESET} ${BOARD_NAME}"
			fi
		fi
	else
		...
		if [ -n "${BIOS_DATE}" ]; then
			if [ -n "${BIOS_VERSION}" ]; then
				if [ -n "${BIOS_VENDOR}" ]; then
					echo -e "${COL}${BOLD}└─Bios:${RESET} ${BIOS_DATE}, version ${BIOS_VERSION} (${BIOS_VENDOR})"
				else
					echo -e "${COL}${BOLD}└─Bios:${RESET} ${BIOS_DATE}, version ${BIOS_VERSION}"
				fi
			elif [ -n "${BIOS_VENDOR}" ]; then
				echo -e "${COL}${BOLD}└─Bios:${RESET} ${BIOS_DATE} (${BIOS_VENDOR})"
			else
				echo -e "${COL}${BOLD}└─Bios:${RESET} ${BIOS_DATE}"
			fi
		fi
	else
...
if [[ -n "$GET_C_CPU_INFO" || -n "$GET_ALL_INFO" ]]; then
	if ! command -v lscpu >> /dev/null ; then	## If lscpu not installed (send to /dev/null to suppress stdout)
		...
		if [ -n "${CPU_MODEL}" ];	then	echo -e "${COL}${BOLD}├─Model:${RESET} ${CPU_MODEL}"; fi	## CPU model.
		if [ -n "${CPU_VENDOR}" ];	then	echo -e "${COL}${BOLD}├─Vendor:${RESET} ${CPU_VENDOR}"; fi	## CPUM vendor.
...
if [[ -n "$GET_A_AUDIO_INFO" || -n "$GET_V_VIDEO_INFO" || -n "$GET_ALL_INFO" ]]; then
	if ! command -v lspci >> /dev/null || ! lspci &> /dev/null; then		## If lspci not installed or fails.
		...
		if [[ -n "$GET_ALL_INFO" || ( -n "$GET_A_AUDIO_INFO" && -n "$GET_V_VIDEO_INFO" ) ]]; then	## If we want both audio and video info.
...
if [[ -n "$GET_D_DISKS_INFO" || -n "$GET_ALL_INFO" ]]; then
	if ! command -v lsblk >> /dev/null ; then	## If lsblk not installed (send to /dev/null to suppress stdout).
		...
		do
			...
			if [ "$(lsblk -dno TYPE /dev/"${WORKING_DEVICE}")" = "loop" ]; then continue; fi	## Skip output if it's a "loop" (for a snap).
			...
			for (( dspec=0; dspec<${#DEV_SPECS[@]}; dspec++ ))							## Loop for each spec.
			do
				if (( dspec==(${#DEV_SPECS[@]}-1) )) && (( NUM_CHILDREN==1 )); then	C2="${COL}└─${RESET}"	## If last spec, no children.
			...
			do
				...
				if [ -e /dev/"${WORKING_CHILD}" ]; then		## Only proceed for this "working_child" if it is a discrete device.
					...
					do
						if (( cspec==(${#CHILD_SPECS[@]}-1) )) && (( NUM_GRANDCHILDREN==1 )); then	C3="${COL}${DIM}└─${RESET}"	## Last child spec.
					...
					do
						...
						for (( gcspec=0; gcspec<${#GRANDCHILD_SPECS[@]}; gcspec++ ))					## Loop for each grandchild spec.
						do
...
if [[ -n "$GET_N_NETWORK_INFO" || -n "$GET_ALL_INFO" ]]; then
	...
	if ! command -v nmcli >> /dev/null ; then	DNS=$(grep -m 1 "nameserver" /etc/resolv.conf | cut -d " " -f 2)		## Grep first DNS from resolv.conf.
	else 						DNS=$(nmcli dev show | grep -m 1 "DNS" | tr -s " " | cut -d " " -f 2); fi	## Preferably determine DNS with nmcli.
	...
	do
		...
		if (( c==NUM_DEVS )); then	echo -e "${COL}${BOLD}└─Interface:${RESET} ${WORKING_INTERFACE}" && C1="${COL}${BOLD}  ${RESET}"	## If it's the last interface.
		...
		if [ "${ESSID}" == "Wired" ] ; then ESSID=""; fi	## Clear essid if it was identified as "Wired".
