...
##########Exit codes
SUCCESS="0"		## Noice.
BAD_LOGFILE="1"		## Cannont write to log file.
...
if [ ! -w "$(dirname "${BU_LOG_FILE}")" ]; then
	if ! mkdir --parents "$(dirname "${BU_LOG_FILE}")" || ! touch "${BU_LOG_FILE}"; then
...
##########Validate argument
ARGUMENT=${1-"$(dirname "$0")/bu.list"}		## First argument is the file name of the file list a specific file to be backed up.
...
elif ssh "${SSH_OPTIONS[@]}" "${BU_USER}@${BU_SERVER_REMOTE}" "exit" > ${DEST} 2>&1; then	## Else if connected to remote server...
...
if [ "${ARGUMENT_TYPE}" == "FILE" ]; then	## Expecting a specific file/directory.
	...
	if command -v file >> /dev/null && ! file "${ARGUMENT}" | grep "ASCII text" >> /dev/null; then
		...
		while read -r LINE ; do		## Iterate for every line in the backup file list.
			STRIPPED_LINE=$(printf "%s" "${LINE}" | tr -s " " | tr -d "\t" | cut -d "#" -f 1)	## Strip comments.
			if [ "${STRIPPED_LINE}" ]; then	
				if [ "$(printf "%s" "${STRIPPED_LINE}" | cut -b ${#STRIPPED_LINE})" == " " ]; then ## Space at end.
					STRIPPED_LINE="$(printf "%s" "${STRIPPED_LINE}" | cut --complement -b ${#STRIPPED_LINE})";
				fi
