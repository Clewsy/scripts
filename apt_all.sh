...
while read -r REM_SYS <&2; do	## Loop to repeat commands for each file name entry in the backup file list ($BU_FILE_LIST).
	...
	if ! ssh -o "BatchMode=yes" -o "ConnectTimeout=4" "${REM_SYS}" "exit" > /dev/null 2>&1; then	## Test ssh connection to current host machine.  If it fails...
		{
			...
			echo -E "${BOLD}║${RESET}Skipping.\t\t\t${BOLD}║${RESET}"
			echo -E "${BOLD}╠═══════════════════════════════╣${RESET}"
	...
	###### Attempt update.
	if ! ssh -o "BatchMode=yes" "${REM_SYS}" "sudo apt-get -y update"; then
		{
	...
	if ! ssh -o "BatchMode=yes" "${REM_SYS}" "sudo apt-get -y --show-progress dist-upgrade"; then
		{
	...
	###### Attempt autoremove.
	if ! ssh -o "BatchMode=yes" "${REM_SYS}" "sudo apt-get -y --show-progress autoremove"; then
		{
	...
	###### Attempt autoclean.
	if ! ssh -o "BatchMode=yes" "${REM_SYS}" "sudo apt-get -y autoclean"; then
		{
